...
// مپ کردن کلیدهای منو به مقادیر واقعی با en-dash
const keyToValueMap: Record<string, { field: keyof Technology; value: string }> = {
  ...
  supply_chain_stage_1: { field: 'supplyChainStage', value: 'First‑Mile Logistics' },  // en-dash
  supply_chain_stage_2: { field: 'supplyChainStage', value: 'Last‑Mile Logistics' },   // en-dash
  supply_chain_stage_3: { field: 'supplyChainStage', value: 'Mid‑Mile Logistics' },    // en-dash
  trade_channel_type_1: { field: 'tradeChannelType', value: 'Electronic Commerce (E‑commerce)' },  // en-dash
  trade_channel_type_2: { field: 'tradeChannelType', value: 'Mobile Commerce (M‑commerce)' },     // en-dash
  ...
  trade_channel_type_7: { field: 'tradeChannelType', value: 'Direct‑to‑Consumer (D2C) Model' },  // en-dash
  trade_channel_type_8: { field: 'tradeChannelType', value: 'Omni‑channel Commerce' },           // en-dash
  trade_channel_type_9: { field: 'tradeChannelType', value: 'Traditional Commerce (Offline)' },
  industry_1: { field: 'industry', value: 'Retail – Online & Offline' },  // en-dash
...
// تابع فیلتر چندگانه: OR درون فیلد، AND بین فیلدها
const filterTechnologies = (technologies: Technology[], selectedKeys: string[]): Technology[] => {
  ...
  // گروه‌بندی کلیدها بر اساس field
  ...
  selectedKeys.forEach((key) => {
    ...
    if (mapping) {
      ...
      if (!groupedFilters[field].includes(mapping.value)) {
        groupedFilters[field].push(mapping.value);
      }
    }
  });

  // فیلتر: تکنولوژی باید با همه فیلدها مطابقت داشته باشه (AND)، و درون هر فیلد با حداقل یکی (OR با includes)
  return technologies.filter((tech) => {
    return Object.entries(groupedFilters).every(([field, values]) => {
      return values.includes(tech[field as keyof Technology] as string);
    });
  });
};
...
export function RadarChart({
  ...
  // محاسبه تکنولوژی‌های فیلترشده
  const filteredTechnologies = React.useMemo(() => {
    return filterTechnologies(technologies, selectedKeys);
  }, [technologies, selectedKeys]);

  // محاسبات داینامیک برای آمارها
  const totalTechnologies = React.useMemo(() => filteredTechnologies.length, [filteredTechnologies]);
  const highImpactTechnologies = React.useMemo(
    () => filteredTechnologies.filter((tech) => tech.impact >= 75).length,
    [filteredTechnologies]
  );
  const nearTermTechnologies = React.useMemo(
    () => filteredTechnologies.filter((tech) => tech.timeline <= 3).length,
    [filteredTechnologies]
  );
  ...
  const placeLabelWithCollision = (
    ...
    directions.forEach((dir) => {
      for (let offsetLevel = 0; offsetLevel < 4; offsetLevel++) {
        const offset = baseOffset + offsetLevel * 15;
    ...
    while (best.score > overlapThreshold && attempts < 5) {
      ...
      const newScore = calculateOverlapScore(
        { x: best.labelX - textWidth / 2 - padding, y: best.labelY - textHeight / 2 - 1, width: boxWidth, height: boxHeight },
        occupiedLabels
      );
    ...
    if (best.score > 0) {
      const adjustOffset = 5 + Math.random() * 5;
      ...
      best.score = calculateOverlapScore(
        { x: best.labelX - textWidth / 2 - padding, y: best.labelY - textHeight / 2 - 1, width: boxWidth, height: boxHeight },
        occupiedLabels
      );
    }
  ...
  function calculateOverlapScore(newBox: any, occupied: any[]) {
    ...
    for (let existing of occupied) {
      ...
      const overlapX = Math.max(0, (newBox.width / 2 + existing.width / 2) + 2 - dx); // +2 buffer
      const overlapY = Math.max(0, (newBox.height / 2 + existing.height / 2) + 2 - dy); // +2 buffer
      if (overlapX > 0 && overlapY > 0) {
        score += (overlapX * overlapY) / 50; // penalize سخت‌تر (50 به جای 100)
      }
    }
    return score;
  }
  ...
  useEffect(() => {
    ...
    const renderRadar = () => {
      ...
      // استفاده از filteredTechnologies
      ...
      sortedTechnologies.forEach((tech) => {
        ...
        const textWidth = textMetrics.width * (visualScale / baseScale);
        ...
        ctx.font = `${isSelected || isHovered ? 'bold ' : ''}${textHeight}px Inter, sans-serif`;
        ctx.textAlign = 'center';
  ...
  const handleCanvasClick = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const clickedTech = filteredTechnologies.find((tech) => {
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const x = (event.clientX - rect.left - panOffset.x) / zoomLevel;
      const y = (event.clientY - rect.top - panOffset.y) / zoomLevel;
      const hoveredTech = filteredTechnologies.find((tech) => {
        const pos = techPositions[tech.id];
        if (!pos) return false;
        const distance = Math.sqrt((x - pos.x) ** 2 + (y - pos.y) ** 2);
        return distance <= 20 / zoomLevel;
    ...
    [isDragging, dragStart, panOffset, zoomLevel, onPanChange, onTechnologyHover, filteredTechnologies, techPositions]
  );
